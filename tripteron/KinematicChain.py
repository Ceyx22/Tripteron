    ...
    def fkin(self, q):
        ...
        for s in self.steps:
            ...
            if s.type is Joint.REVOLUTE:
                ...
                T_joint = s.Tshift @ T_from_Rp(Rote(s.elocal, q[s.dof]), pzero())
            elif s.type is Joint.LINEAR:
                # Linear is a translation:
                T_joint = s.Tshift @ T_from_Rp(Reye(), pe(s.elocal, q[s.dof]))
            else:
        ...
        for s in self.steps:
            # AGAIN.  From the above, the step now includes:
